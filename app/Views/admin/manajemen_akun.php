...
<div class="container-fluid">
    ...
    <div class="card shadow-sm mb-4" style="background: rgba(30, 41, 59, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; backdrop-filter: blur(10px);">
        <div class="card-header border-0 pb-0 d-flex justify-content-between align-items-center" style="background: transparent;">
            <h5 class="mb-0 text-white"><i class="fas fa-users-cog text-danger me-2"></i> Daftar Akun Pengguna</h5>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-dark table-hover table-borderless align-middle" id="dataTable" width="100%" cellspacing="0" style="background: transparent;">
    ...
    <div class="card shadow-sm" style="background: rgba(30, 41, 59, 0.8); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; backdrop-filter: blur(10px);">
        <div class="card-header border-0 pb-0 d-flex justify-content-between align-items-center" style="background: transparent;">
            <h5 class="mb-0 text-white">
                <i class="fas fa-key text-danger me-2"></i> Redeem Code Pendaftaran Kreator
            </h5>
            <div class="d-flex align-items-center gap-2">
                <span class="text-muted small me-2" style="font-size: 0.7rem;">
                    ...
                    <button onclick="copyLink('<?= $register_url ?>')" class="btn btn-sm p-0 ms-1 text-muted border-0" title="Salin Link" style="background:none;">
        ...
        <div class="card-body">
            <?php if (empty($redeem_codes)): ?>
                <p class="text-muted text-center small py-3">
                    <i class="fas fa-inbox me-2"></i>Belum ada Redeem Code. Masukkan jumlah dan klik **Generate** di atas untuk membuat baru.
                </p>
            <?php else: ?>
                <?php if (!empty($unused_links_str)): ?>
                    ...
                    <div class="mb-3 d-flex justify-content-start">
                        <button onclick="copyFromTextarea('unusedLinksText', this, 'bulkCopyLinkIcon', 'Salin Semua Link (<?= $unused_codes_count ?>)')"
                                ...
                                style="border-color: rgba(0, 191, 255, 0.4); color: #00bfff; font-weight: bold; background: rgba(0, 191, 255, 0.05); padding: 6px 12px; border-radius: 4px;">
                        ...
                        <tbody>
                            <?php foreach ($redeem_codes as $i => $rc): ?>
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                                    <td><?= $i + 1 ?></td>
                                    <td>
                                        <code style="font-size: 1rem; color: <?= $rc['is_used'] ? '#64748b' : '#ea1917' ?>; letter-spacing: 2px; background: rgba(255,255,255,0.05); padding: 3px 10px; border-radius: 3px;">
                                            <?= esc($rc['code']) ?>
                                        </code>
                                        <?php if (!$rc['is_used']): ?>
                                            <button onclick="copyLink('<?= $register_url . '?code=' . esc($rc['code']) ?>', this)"
                                                    class="btn btn-sm p-0 ms-2 text-muted border-0"
                                            ...
                                            <a href="https://api.whatsapp.com/send?text=<?= urlencode($waText) ?>"
                                               target="_blank"
                                               class="btn btn-sm p-0 ms-2 text-success border-0"
                                    ...
                                    <td>
                                        <?php if ($rc['is_used']): ?>
                                            <span class="badge" style="background: rgba(100,116,139,0.3); color: #94a3b8; border: 1px solid #475569;">
                                                <i class="fas fa-check me-1"></i> Sudah Dipakai
                                            </span>
                                        <?php else: ?>
                                    ...
                                    <td class="text-center">
                                        <form method="POST" action="<?= base_url('admin/users/delete-code/' . $rc['redeem_id']) ?>" style="display:inline-block; margin:0;">
                                            <?= csrf_field() ?>
                                            <button type="button" class="btn btn-outline-danger btn-sm p-1 border-0 btn-delete-code" title="Hapus Kode" data-confirm-text="<?= $rc['is_used'] ? 'Kode ini sudah dipakai oleh kreator. Yakin ingin tetap menghapus riwayat kode ini?' : 'Yakin ingin menghapus/menonaktifkan kode ini?' ?>">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </form>
                                    </td>
