...
class LaporanMingguanModel extends Model
{
    ...
    public static function calculateTier($metrics): array
    {
        ...
        $yt  = (int)($metrics['yt_avg'] ?? 0);
        $yts = (int)($metrics['yt_shorts_avg'] ?? 0); // NEW: YT Shorts Average
        $tt  = (int)($metrics['tt_avg'] ?? 0);

        // Ambil Salah Satu Terbaik dari YouTube (Reguler atau Shorts)
        $yt_best = max($yt, $yts);

        // Tier 1
        if ($ccv >= $t1_ccv && ($yt_best >= $t1_yt || $tt >= $t1_tt)) {
            return [
                'name'  => 'Tier 1',
                'label' => 'Tier 1',
        ...
        // Tier 2
        if ($ccv >= $t2_ccv && ($yt_best >= $t2_yt || $tt >= $t2_tt)) {
            return [
                'name'  => 'Tier 2',
                'label' => 'Tier 2',
                'icon'  => 'fas fa-check-circle',
        ...
        // Tier 3
        if ($ccv >= $t3_ccv && ($yt_best >= $t3_yt || $tt >= $t3_tt)) {
            return [
                'name'  => 'Tier 3',
                'label' => 'Tier 3',
                'icon'  => 'fas fa-check-circle',
    ...
    public function getPersonalStats(int $kreatorId)
    {
        ...
        if (!empty($currReports)) {
            $rPeak = 0; $rYtV = 0; $rYtC = 0; $rYtSV = 0; $rYtSC = 0; $rTtV = 0; $rTtC = 0;
            foreach ($currReports as $rl) {
                $rPeak = max($rPeak, $rl['penonton_puncak_live']);
                if ($rl['platform'] == 'youtube') {
                    $rYtV  += $rl['total_views_video']; $rYtC += $rl['jumlah_video'];
                    $rYtSV += $rl['views_shorts'];      $rYtSC += $rl['jumlah_shorts'];
                } else {
                    $rTtV += $rl['total_views_video'];  $rTtC += $rl['jumlah_video'];
                }
            }
            $stats['recent_metrics'] = [
                'peak_ccv'      => $rPeak,
                'yt_avg'        => $rYtC > 0 ? ($rYtV / $rYtC) : 0,
                'yt_shorts_avg' => $rYtSC > 0 ? ($rYtSV / $rYtSC) : 0,
                'tt_avg'        => $rTtC > 0 ? ($rTtV / $rTtC) : 0
            ];
        }
